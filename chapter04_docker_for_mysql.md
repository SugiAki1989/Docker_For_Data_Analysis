...
起動時の設定をまとめて記述しておける`docker-compose.yml`の中身は下記の通りです。`- ./init:/docker-entrypoint-initdb.d`の部分がポイントで、新しいコンテナが初めて起動されると、指定された名前の新しいデータベースが作成され、提供された構成変数で初期化されます。このマウント先のsql、shなどを実行してくれます。ここでは、`init.sql`が実行されます。
...
`init.sql`の中身は下記の通りです。データベースを作り、テーブルにデータをインサートします。
